...
2. Reaplicar correções de bug que se perderam no alinhamento PC→Mac:
   ...
   - `cloudApply` chamar `ajustarCounters` → **confirmado ausente** no código atual; baixo risco, candidato a portar.
   - `htStreak` usar `htDone` → **NÃO portar às cegas**: o `htStreak` atual calcula a sequência via `htLogs` e funciona; investigar qual bug a versão do PC resolvia antes de mexer.
